-- Variables got fix
    ...
    local touch_service = game:GetService("TouchInputService")
...
-- Library init
    ...
    getgenv().library = {
        ...
        gui; 
        is_mobile = uis.TouchEnabled and not uis.MouseEnabled,
    }
        ...
        function library:initializeColorPicker(options) 
            ...
            -- Functions 
                function cfg.updateColor() 
                    local touch_pos = uis:GetTouchLocation()
...
--[[
        function library:keyPicker(options) 
            ...
            local function updateUI()
                local hueValue = 1 - h
                local hueOffset = (hueValue < 1 and hueValue > 0) and -2 or (hueValue <= 0 and 0 or -4)
                hue_picker.Position = dim2(0, 0, hueValue, hueOffset)

                local alphaOffset = (a < 1 and a > 0) and -2 or (a <= 0 and 0 or -4)
                alpha_picker.Position = dim2(0, 0, a, alphaOffset)
                alpha_drag.BackgroundColor3 = hsv(h, s, v)

                local sOffset = (s < 1 and s > 0) and -3 or (s <= 0 and 0 or -3)
                local vOffset = (1 - v < 1 and 1 - v > 0) and -3 or ((1 - v) <= 0 and 0 or -3)
                dragging_sat_val.Position = dim2(s, sOffset, 1 - v, vOffset)
            ...
            local function applyToTarget()
                ...
                if options.colorPath then
                    options.colorPath.BackgroundColor3 = finalColor
                end
                ...
                function cfg.updateColor() 
                    ...
                    if draggingSaturation then
                        local x = (pos.X - sat.AbsolutePosition.X) / sat.AbsoluteSize.X
                        local y = (pos.Y - sat.AbsolutePosition.Y) / sat.AbsoluteSize.Y
                    ...
                    if draggingHue then
                        local y = (pos.Y - hue.AbsolutePosition.Y) / hue.AbsoluteSize.Y
                    ...
                    if draggingAlpha then
                        local y = (pos.Y - alpha.AbsolutePosition.Y) / alpha.AbsoluteSize.Y
       ...
       --]]
        function library:keyPicker(options) 
            ...
            local function updateDisplay()
                ...
                val.BackgroundColor3 = Color3.fromHSV(h, 1, 1)
                alpha_drag.BackgroundColor3 = Color3.fromHSV(h, s, v)
                
                dragging_sat_val.Position = UDim2.new(s, -3, 1 - v, -3)
                hue_picker.Position = UDim2.new(0, -2, 1 - h, -2)
                alpha_picker.Position = UDim2.new(0, -2, a, -2)
            end
            ...
            local function handleDrag(frame, callback)
                frame.InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                        local conn
                        conn = uis.InputChanged:Connect(function()
                            local mouse = uis:GetMouseLocation()
                            local x = (mouse.X - frame.AbsolutePosition.X) / frame.AbsoluteSize.X
                            local y = (mouse.Y - frame.AbsolutePosition.Y) / frame.AbsoluteSize.Y
                            callback(x, y)
                            updateColor()
                        end)
            ...
            handleDrag(sat, function(x, y)
                s = math.clamp(x, 0, 1)
                v = 1 - math.clamp(y, 0, 1)
            end)

            handleDrag(hue, function(_, y)
                h = 1 - math.clamp(y, 0, 1)
            end)

            handleDrag(alpha, function(_, y)
                a = math.clamp(y, 0, 1)
            end)
    ...
    -- Library element functions
        function library:window(properties)
            local cfg = {
                ...
                size = properties.size or properties.Size or dim2(0, library.is_mobile and 400 or 516, 0, library.is_mobile and 600 or 563),
                selected_tab, 
        ...
        function library:tab(properties)
            ...
            Padding = dim(0, 11)
                ...
                local left_section_frame = library:create("Frame", {
                    Parent = cfg["page"],
                    Name = "LeftSection",
                    Position = dim2(0, 11, 0, 11),
                    Size = dim2(0.5, -16, 0, 0),
                    BackgroundColor3 = rgb(13, 13, 13),
                    BorderSizePixel = 0,
        ...
        function library:addSlider(options) 
            ...
            -- Connections
                ...
                slider_dragger.MouseButton1Down:Connect(onDragStart)
                
                if library.is_mobile then
                    slider_dragger.TouchLongPress:Connect(onDragStart)
                end
                ...
                library:connection(uis.InputChanged, function(input)
                    if cfg.dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then 
                        local pos = library.is_mobile and input.Position or input.Position
                        local size_x = (pos.X - slider_dragger.AbsolutePosition.X) / slider_dragger.AbsoluteSize.X
            ...
            -- Functions 
                ...
                function cfg.refreshOptions(list) 
                    ...
                    for _, option in next, list do 
                        ...
                        OPTION_INSTANCE.MouseButton1Down:Connect(onOptionClick)
            ...
            -- Connections 
                ...
                textbox.FocusLost:Connect(function()
                    if textbox.Text == "" then
                        textbox.Text = cfg.placeholder
                    end
                end)

                textbox.Focused:Connect(function()
                    if textbox.Text == cfg.placeholder then
                        textbox.Text = ""
                    end
                end)
            -- 
        ...
        function library:addKeyBind(options) 
            ...
            -- Connections 
                ...
                outline.MouseButton1Down:Connect(onBindStart)
                
                if library.is_mobile then
                    outline.TouchLongPress:Connect(onBindStart)
                end
